...
// Uses Frankfurter (ECB daily reference rates, free, no key, no rate limit)
// Falls back to the most recent prior business day if the requested date has no rate
// (weekends / public holidays aren't published by the ECB)
export default async function handler(req, res) {
  ...
  try {
    // Try the requested date first, walking backwards up to 7 days if needed
    ...
    if (rate === null) {
      return res.status(502).json({ error: "No FX rate found within 7 days of requested date" });
    }
